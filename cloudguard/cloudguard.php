...
function cloudguard_options_section_callback() {
	?><p><?php printf(__('Enter the <a href="%s" target="_blank" rel="noopener">2 digit code</a> of the country you wish to <b>ALLOW</b> access from.', 'cloudguard'), esc_url('https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2#Officially_assigned_code_elements')); ?></p><?php
...
function cloudguard_dash_widget_func() {
	...
	if (!empty($blocked_attempts)) {
		...
		foreach ($blocked_attempts as $country => $attempts) {
			...
			$flag_img_src = plugin_dir_url(__FILE__).'assets/flags/'.strtolower($country).'.png';
			
			if (!file_exists($flag_img_src)) {
				$flag_img_src = plugin_dir_url(__FILE__).'assets/flags/xx.png';
			}
			
			echo '<p><img class="cg_flag" src="'.esc_url($flag_img_src).'" alt=""/> '.cloudguard_code_to_country($country).' ('.absint($attempts).')</p>';
